...
const [ID, POCET = "6", RAD = "standard"] = process.argv.slice(2);
const SUBOR = "app/products.ts";
const FOTKY = Array.from({ length: Number(POCET) }, (_, i) => `/img/products/${RAD}-120x40x80-${ID}-${String(i + 1).padStart(2, "0")}.webp`);
...
const vsetky = PRODUCTS.map((p) => {
  if (p.tier !== RAD || p.w < 120) return p;
  ...
  if (p.slug !== `${RAD}-120x40x80`) Object.assign(novy, { inherited: true, illuFrom: "rozmer", illuSize: "120 × 40 × 80 cm" });
  n++;
